...
object JpegEncoder:
  ...
  /** Encodes an RGB image as a three-component, full-resolution (4:4:4) JFIF
    * stream. Keeping 4:4:4 as the first color milestone makes MCU ordering
    * visible before introducing chroma resampling.
    */
  def encode(image: RgbImage, options: EncoderOptions): IArray[Byte] =
    val quantizer = options.quality.scale(Quantization.Luminance)
    val converted = (for y <- 0 until image.height; x <- 0 until image.width yield YCbCr.fromRgb(image(x, y)))
    ...
    segment(out, 0xe0, Seq(0x4a,0x46,0x49,0x46,0,1,1,0,0,1,0,1,0,0))
    segment(out, 0xdb, 0 +: Quantization.zigZag(quantizer))
    segment(out, 0xc0, Seq(8) ++ u16(image.height) ++ u16(image.width) ++
      Seq(3, 1,0x11,0, 2,0x11,0, 3,0x11,0))
  ...
  private def colorEntropy(components: IndexedSeq[GrayImage], quantizer: Block): IArray[Byte] =
    ...
    for blockIndex <- blocks.head.indices; component <- components.indices do
      previousDc(component) = writeBlock(blocks(component)(blockIndex), quantizer, previousDc(component), bits)
    bits.result()
